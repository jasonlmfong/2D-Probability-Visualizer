...
Mesh::Mesh(const HeightMap& xHeightMap, const HeightMap& yHeightMap)
{
    Generate(xHeightMap, yHeightMap);
}
...
void Mesh::Generate(const HeightMap& xHeightMap, const HeightMap& yHeightMap)
{
    int mapWidth = xHeightMap.m_Size;
    int mapLength = yHeightMap.m_Size;
    ...
    for (int j = 0; j < mapLength; j++)
    {
        ...
        for (int i = 0; i < mapWidth; i++)
        {
            ...
            m_VertexPos[j][i] = glm::vec3{ 
                ...
                yHeightMap.m_Map[j] * xHeightMap.m_Map[i],
...
void Mesh::Regenerate(const HeightMap& xHeightMap, const HeightMap& yHeightMap)
{
    Destroy();
    Generate(xHeightMap, yHeightMap);
}
